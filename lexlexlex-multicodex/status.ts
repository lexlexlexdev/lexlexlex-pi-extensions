...
const NO_FAST_MODE: FastModeStatus = {
  isEffective: () => false,
}

export type PercentDisplayMode = 'left' | 'used' | 'raw'
...
function isPercentDisplayMode(value: unknown): value is PercentDisplayMode {
  return value === 'left' || value === 'used' || value === 'raw'
}
...
function usedToDisplayPercent(
  ...
  return mode === 'used' ? clampPercent(100 - left) : left
}
...
function formatPercent(
  ...
  const percent = `${Math.round(clampPercent(displayPercent))}%`
  return mode === 'raw' ? percent : `${percent} ${mode}`
}
...
function formatFastIndicator(
  ctx: ExtensionContext,
  enabled: boolean,
): string | undefined {
  return enabled ? ctx.ui.theme.fg('error', 'fast') : undefined
}
...
export function formatActiveAccountStatus(
  ...
  const accountText = preferences.showAccount
    ? ctx.ui.theme.fg('text', accountAlias?.trim() || accountEmail)
    : undefined
  const fastIndicator = formatFastIndicator(ctx, fastEnabled)
  if (!usage) {
    return joinFooterElements(ctx, [accountText, formatLoading(ctx), fastIndicator])
  }

  if (preferences.usageMode === 'raw') {
    ...
    const weekly = formatUsageSegment(
      ctx,
      RAW_WEEK_LABEL,
    ...
    return joinFooterElements(ctx, [accountText, `${fiveHour}, ${weekly}`, fastIndicator])
  }
  ...
  const elements =
    preferences.order === 'account-first'
      ...
      : [usageText, accountText]
  elements.push(fastIndicator)
  return joinFooterElements(ctx, elements)
}
  ...
  return [
    {
      ...
      label: 'Usage display',
      description:
        'Show remaining, consumed, or raw quota percentages without labels',
      currentValue: preferences.usageMode,
      values: ['left', 'used', 'raw'],
    },
    {
      id: 'resetWindow',
...
export function createUsageStatusController(
  accountManager: AccountManager,
  fastMode: FastModeStatus = NO_FAST_MODE,
) {
  ...
  function getStatusText(
    ...
    if (!contextHasUI(ctx)) return undefined
    ...
    return formatActiveAccountStatus(
      ...
      activeAccount.alias,
      fastMode.isEffective(ctx.model),
    )
  }
  ...
  async function updateStatus(ctx: ExtensionContext): Promise<void> {
    ...
    const usage =
      (await accountManager.refreshUsageForAccount(activeAccount, { force: true })) ??
      cachedUsage
    ctx.ui.setStatus(
      STATUS_KEY,
  ...
  function renderPreviewLabel(
    ...
    const previewText =
      getStatusText(ctx, draft) ?? formatLoading(ctx)
