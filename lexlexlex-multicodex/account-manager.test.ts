...
vi.mock('./provider', () => ({
  getOpenAICodexOAuth: () => mocks.oauth,
}))
...
describe('AccountManager account aliases', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.storageData.accounts = []
    mocks.storageData.activeEmail = undefined
  ...
  it('sets, trims, and clears a managed account alias', () => {
    const manager = new AccountManager()
    const account = manager.addOrUpdateAccount('work@example.com', {
      type: 'oauth',
      access: 'access',
      refresh: 'refresh',
      expires: Date.now() + 3600_000,
    ...
    expect(manager.setAccountAlias(account.email, '  work  ')).toBe(true)
    expect(manager.getAccount(account.email)?.alias).toBe('work')
    expect(mocks.saveStorage).toHaveBeenCalled()

    expect(manager.setAccountAlias(account.email, '   ')).toBe(true)
    expect(manager.getAccount(account.email)?.alias).toBeUndefined()
  })

  it('promotes an ephemeral pi account when it receives a persistent alias', async () => {
    mocks.loadImportedOpenAICodexAuth.mockResolvedValue({
      identifier: 'pi@example.com',
      fingerprint: 'fp',
      credentials: {
    ...
    await manager.loadPiAuth()
    const account = manager.getAccount('pi@example.com')
    expect(account && manager.isPiAuthAccount(account)).toBe(true)

    expect(manager.setAccountAlias('pi@example.com', 'personal')).toBe(true)
    const promoted = manager.getAccount('pi@example.com')
    expect(promoted).toMatchObject({ alias: 'personal' })
    expect(promoted && manager.isPiAuthAccount(promoted)).toBe(false)
    expect(mocks.saveStorage).toHaveBeenCalled()
  })
})
...
describe('AccountManager usage fetch warnings', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.storageData.accounts = []
    mocks.storageData.activeEmail = undefined
  ...
  it('fails silently for aborted usage fetches', async () => {
    const manager = new AccountManager()
    const warningHandler = vi.fn()
    manager.setWarningHandler(warningHandler)
    ...
    mocks.fetchCodexUsage.mockRejectedValueOnce(abortError)
  ...
  it('warns once until usage fetch recovers', async () => {
    const manager = new AccountManager()
    const warningHandler = vi.fn()
    manager.setWarningHandler(warningHandler)
    const account = manager.addOrUpdateAccount('warn@example.com', {
    ...
    expect(warningHandler).toHaveBeenCalledTimes(1)
    expect(warningHandler.mock.calls[0]?.[0]).toContain(
      'failed to fetch usage for warn@example.com: boom',
    )
    ...
    await manager.refreshUsageForAccount(account, { force: true })
    expect(warningHandler).toHaveBeenCalledTimes(2)
    expect(warningHandler.mock.calls[1]?.[0]).toContain(
      'failed to fetch usage for warn@example.com: boom again',
    )
  })
})
