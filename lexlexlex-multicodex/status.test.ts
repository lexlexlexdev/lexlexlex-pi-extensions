...
describe('formatActiveAccountStatus', () => {
  ...
  it('uses an account alias in the footer', () => {
    const ctx = createContext()
    const text = formatActiveAccountStatus(
      ctx,
      'a@example.com',
      {
        primary: { usedPercent: 25, resetAt: 1 },
        secondary: { usedPercent: 60, resetAt: 2 },
        fetchedAt: 0,
      },
      defaultPreferences,
      'work',
  ...
  it('renders the account alias before raw remaining percentages', () => {
    const ctx = createContext()
    const text = formatActiveAccountStatus(
      ctx,
      'a@example.com',
    ...
    expect(text).toBe('work · 5h:75%, W:40%')
    expect(text).toContain('work')
    expect(text).not.toContain('a@example.com')
    expect(text).not.toContain('Codex')
  ...
  it('shows Fast after usage in error color and hides it when disabled', () => {
    const ctx = createContext({
    ...
    const enabled = formatActiveAccountStatus(
      ctx,
      'a@example.com',
      usage,
      defaultPreferences,
    ...
    const disabled = formatActiveAccountStatus(
      ctx,
      'a@example.com',
      usage,
      defaultPreferences,
    )
    const raw = formatActiveAccountStatus(
      ctx,
      'a@example.com',
      usage,
      { ...defaultPreferences, usageMode: 'raw' },
      'work',
      true,
    )

    expect(enabled).toMatch(/\[error:fast\]$/)
    expect(enabled.indexOf('[error:fast]')).toBeGreaterThan(
      enabled.indexOf('7d:'),
    )
    expect(disabled).not.toContain('fast')
    expect(raw).toContain(
      '[text:work] [muted:·] [success:5h:75%], [thinkingMedium:W:40%]',
    )
    expect(raw).toMatch(/\[muted:·\] \[error:fast\]$/)
  })
  ...
  it('uses muted loading text and dim unknown usage windows', () => {
    ...
    expect(loading).not.toContain('Codex')
