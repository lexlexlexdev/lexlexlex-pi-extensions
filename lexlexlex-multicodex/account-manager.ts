...
import { getOpenAICodexOAuth } from './provider'
...
export class AccountManager {
  ...
  constructor(oauth: OAuthAuth = getOpenAICodexOAuth()) {
    this.oauth = oauth
  ...
  private removeAccountRecord(account: Account): boolean {
    ...
    if (removedEmail) {
      ...
      this.warnedUsageFailureEmails.delete(removedEmail)
  ...
  private applyCredentials(account: Account, creds: OAuthCredential): boolean {
    const accountId =
  ...
  addOrUpdateAccount(email: string, creds: OAuthCredential): Account {
  ...
  setAccountAlias(email: string, alias: string): boolean {
    const account = this.getAccount(email)
    if (!account) return false

    const normalized = alias.trim()
    const nextAlias = normalized || undefined
    if (account.alias === nextAlias) return true

    if (this.isPiAuthAccount(account)) {
      // Promote a renamed ephemeral account so the alias survives restart.
      if (!nextAlias) return true
      const managedAccount: Account = { ...account, alias: nextAlias }
      this.data.accounts.push(managedAccount)
      this.piAuthAccount = undefined
      this.data.activeEmail = managedAccount.email
    } else if (nextAlias) {
  ...
  async refreshUsageForAccount(
    ...
    try {
      const token = await this.ensureValidToken(account, options?.signal)
      ...
      this.warnedUsageFailureEmails.delete(account.email)
      this.notifyStateChanged()
      return usage
    } catch (error) {
      if (
        !isAbortLikeError(error) &&
        !this.warnedUsageFailureEmails.has(account.email)
      ) {
        this.warnedUsageFailureEmails.add(account.email)
        this.warningHandler?.(
  ...
  async ensureValidToken(
    ...
    if (this.isPiAuthAccount(account)) {
      return this.ensureValidTokenForPiAuth(account, signal)
    }
    ...
    const promise = (async () => {
      try {
        ...
        this.applyCredentials(account, result)
        this.save()
        this.notifyStateChanged()
        return account.accessToken
      } catch (error) {
   ...
   */
  private async ensureValidTokenForPiAuth(
    account: Account,
    signal?: AbortSignal,
  ): Promise<string> {
    signal?.throwIfAborted()
