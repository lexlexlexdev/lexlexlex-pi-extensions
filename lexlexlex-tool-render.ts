import type { AgentToolResult, ExtensionAPI, Theme, ToolRenderContext, ToolRenderResultOptions } from "@earendil-works/pi-coding-agent";
...
function isNonSafeTool(tool: ToolName, args: any) {
...
function typeLabel(tool: ToolName) {
	switch (tool) {
		case "bash":
			return "sh";
		case "read":
...
function callVerb(tool: ToolName) {
	switch (tool) {
		case "bash":
			return "run";
		case "read":
...
function resultVerb(tool: ToolName, outputLines: number, isError: boolean) {
...
function targetFor(tool: ToolName, args: any, cwd: string) {
	switch (tool) {
		case "bash":
...
function frame(theme: Theme, tool: ToolName, target: string, state: "idle" | "ok" | "err", detail?: string, nonSafe = false) {
	...
	const sub = detail ? `\n  ${dim(theme, detail)}` : "";
	return `${dot(theme, state, nonSafe)} ${kind} ${main}${sub}`;
}
...
function renderCall(tool: ToolName, args: any, theme: Theme, context: ToolRenderContext) {
	...
	const card = frame(theme, tool, targetFor(tool, args, context.cwd), "idle", detail, nonSafe);
...
function renderResult(tool: ToolName, result: AgentToolResult<unknown>, _options: ToolRenderResultOptions, theme: Theme, context: ToolRenderContext) {
...
export default async function lexlexlexToolRender(pi: ExtensionAPI) {
	...
	pi.on("session_start", (_event, ctx) => {
		...
		for (const toolName of TOOL_NAMES) {
			...
			pi.registerTool({
				...tool,
				renderCall: (args, theme, context) => renderCall(toolName, args, theme, context),
				renderResult: (result, options, theme, context) => renderResult(toolName, result, options, theme, context),
			});
		}
	});
}
